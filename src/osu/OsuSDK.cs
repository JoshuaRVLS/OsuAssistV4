...
namespace Osussist.src.osu
{
    public class OsuSDK
    {
        ...
        public bool TryGetPlaybackState(out int currentTime, out bool isPaused)
        {
            currentTime = 0;
            isPaused = false;
            ...
            try
            {
                ...
                currentTime = (int)data.GetType().GetField("MenuTime").GetValue(data);
                isPaused = !(bool)data.GetType().GetField("AudioPlaying").GetValue(data);
                return true;
            }
            catch
            {
        ...
        public int CurrentTime
        {
            get
            {
                return TryGetPlaybackState(out int currentTime, out _) ? currentTime : 0;
            }
        }
        ...
        public bool isPaused
        {
            ...
            {
                if (OsuManager.ProcessManager.ClientType == ClientTypes.Stable)
                    return !TryGetPlaybackState(out _, out bool paused) || paused;
        ...
        {
            return GetRealPlayfieldPosition(hitObject.Position);
        }

        public Vector2 GetHRHitObjectPos(HitObject hitObject)
        {
            return GetHRPlayfieldPosition(hitObject.Position);
        }

        public Vector2 GetRealPlayfieldPosition(Vector2 position)
        {
            Vector2 scaledCoords = position * OsuManager.WindowManager.PlayfieldRatio;
            return OsuManager.WindowManager.PlayfieldPosition + scaledCoords;
        }

        public Vector2 GetHRPlayfieldPosition(Vector2 position)
        {
