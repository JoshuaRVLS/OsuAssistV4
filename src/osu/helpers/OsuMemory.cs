...
namespace Osussist.src.osu.helpers
{
    public class OsuMemory
    {
        ...
        public OsuMemory(Process GivenGameProcess)
        {
            GameProcess = GivenGameProcess;
        }
        ...
        public bool FindPattern(string pattern, out UIntPtr result)
        {
            ...
            {
                if ((uint)region.BaseAddress < (uint)GameProcess.MainModule.BaseAddress)
                    continue;
        ...
        public byte[] ReadMemory(UIntPtr address, uint size)
        {
            ...
            ReadProcessMemory(GameProcess.Handle, address, result, size, out UIntPtr bytesRead);
            return result;
        }
        ...
        {
            UIntPtr bytesRead;
            ReadProcessMemory(GameProcess.Handle, address, buffer, size, out bytesRead);
            return bytesRead;
        }
        #endregion
        ...
        public List<MemoryRegion> EnumerateMemoryRegions()
        {
            ...
            while (VirtualQueryEx(GameProcess.Handle, address, out MEMORY_BASIC_INFORMATION basicInformation, (uint)Marshal.SizeOf(typeof(MEMORY_BASIC_INFORMATION))) != 0)
            {
