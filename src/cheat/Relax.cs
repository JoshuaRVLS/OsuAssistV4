...
namespace Osussist.src.cheat
{
    public class Relax
    {
                        ...
                        else
                        {
                            ...
                            else
                            {
                                if (possibleTime >= CurrentHitObject.StartTime - HitWin50)
                                {
									...
									if (!IsHit && HasExpiredHitWindow(CurrentHitObject, possibleTime))
									{
										logger.Debug("Relax", "Missed hit object, resynchronizing relax.");
									...
									else if (IsHit && possibleTime >= GetReleaseTime(actualTime))
                                    {
                                    ...
                                    else if (!IsHit && hitScanResult == HitScanResult.Wait && HasExpiredHitWindow(CurrentHitObject, possibleTime))
                                    {
		...
		{
			NextObject();
			while (SongIndex < CurrentBeatmap.HitObjects.Count && HasExpiredHitWindow(CurrentHitObject, currentTime))
				NextObject();
		}

		private bool HasExpiredHitWindow(HitObject hitObject, int currentTime)
		{
			int deadline = (hitObject is Spinner) ? hitObject.EndTime + HitWin50 : hitObject.StartTime + HitWin50;
			return currentTime > deadline;
		}

		private int GetReleaseTime(int actualTime)
		{
			int releaseTime = ((CurrentHitObject is HitCircle) ? actualTime : CurrentHitObject.EndTime) + HitObjectTimings.Item2;
			if (CurrentHitObject is HitCircle && SongIndex + 1 < CurrentBeatmap.HitObjects.Count)
				releaseTime = System.Math.Min(releaseTime, CurrentBeatmap.HitObjects[SongIndex + 1].StartTime - 1);
        ...
        private ValueTuple<int, int> RandomizeHitObjectTimings(int index)
        {
            ...
            {
                additionalTiming = timingValue + random.Next(-HitWin300 / 3, HitWin300 / 3 + 1);
            }
            else
            {
                // Sliders must remain held through their tail.
                additionalTiming = random.Next(0, HitWin300 / 2 + 1);
            }
